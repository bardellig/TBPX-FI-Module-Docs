...
The cold box should usually be operated at 15 °C. If the climatic chamber is used, the settings is manual, and it just need to flush dry air. Picture attached, 


The TBPX cold box, attached below, needs the chiller running at a temperature of around 10 °C and the flush of dry air. The chiller is manually configurable. The cold box is configured thanks to the routine implemented in dirigent or through the MQTT (Mosquitto) protocol.  More detail are listed in the manual of the PSI coldbox, here attached. Two terminal are used, one to monitor and one to control the cold box. In short, to use the box, one is obliged to flush dry air by controlling the valve. At the beginning you can control it with rinse and flush commands. When the flux is stable, you can use only the rinse command. The cold box is here shown:   
![coldbox](coldbox.png)

The commands are the latter, for example for controlling a TEC and setting it to certain temperature. The TEC is first controlled with the voltage of the TEC, but the temperature can be set as well. Few lines are shown here. Troubleshoots can happen if the dry air is too hot or the chiller as well. The limits are set around 20 $^{\circ}$C, so it's important to be below that limit. The firmware can only be pushed to TEC and controlled if the chiller and the dry air are flushing, and also the physical interlock must be closed to proper operate the TECs. 
...
An example in the dirigent toml file is attached:
...
[sldo_vi_curves_GADC]
    ...
    delay = 1.0
    step_size = 0.2
    monitors = ["VINA", "VIND", "VDDA", "VDDD"]
```
The expected output file will look like:

The current monitoring makes sense from 3 A for double module. The scan limits are usually 3 to 5 A which are in the operational ball park for double modules. For a high shunt current operation the limit can reach 8 A. It's safer to proceed to high shunt operations with lower temperature on the cold box (climatic chamber). The output for the GADC scan is shown below:
![monitorGADC](sldo_monitor.png)

The output of the needle card scan, is here:
![monitorNeedle](sldo_needle.png)

To use the needle in the cold box a bit more work is needed since the proper adapter has to be used. The adapter is designed to fit also with the clamp that holds the module in contact with the TEC. The proper adapter has less plastic material, and can pass through the adapted support. Four magnets hold down the clamp over the module. The picture of the adapter and the support adapted to it is shown below. 
